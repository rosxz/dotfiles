...
let
  cfg = config.modules.services.xfce;
...
in
{
  ...
  config = mkIf cfg.enable {
    environment.systemPackages = utils.removePackagesByName (with pkgs.unstable; [
      glib # for gsettings
      ...
      ++ lib.optionals (config.services.pipewire.pulse.enable) [
        pavucontrol
    ...
    programs.gnupg.agent.pinentryPackage = mkDefault pkgs.unstable.pinentry-gtk2;
    ...
    services.xserver.desktopManager.session = [{
      ...
      start = ''
        ${pkgs.runtimeShell} ${pkgs.unstable.xfce.xfce4-session.xinitrc} &
        waitPID=$!
      '';
    }];
    ...
    programs.gdk-pixbuf.modulePackages = [ pkgs.unstable.librsvg ];
    ...
    # Systemd services
    systemd.packages = utils.removePackagesByName (with pkgs.unstable.xfce; [
      xfce4-notifyd
    ]) excludePackages;

    security.pam.services.xfce4-screensaver.unixAuth = cfg.enableScreensaver;

    xdg.portal.configPackages = mkDefault [ pkgs.unstable.xfce.xfce4-session ];
  };
}
